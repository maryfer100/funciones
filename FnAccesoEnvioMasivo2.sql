...
BEGIN
	...
	IF EXISTS(SELECT numemp FROM cmcatalogoempleados WHERE numemp = iEmpleado) THEN
		IF EXISTS(SELECT numemp FROM cat_derechosmemocorreomicro WHERE numemp =  iEmpleado) THEN
			SELECT passw INTO sPassw FROM cat_derechosmemocorreomicro WHERE numemp = iEmpleado;
...
--SELECT  FnAccesoEnvioMasivo AS respuesta FROM FnAccesoEnvioMasivo(97597287,'7904');